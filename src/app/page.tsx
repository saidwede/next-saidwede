...
export default function Home() {
  ...
  const { sectionRotate, setSectionRotate, openPos, pos } = uiContext;
  ...
  const handleScroll = () => {
    if (containerRef.current) {
      ...
      console.log(scrollTop, scrollHeight, clientHeight, pos)
      if((scrollTop + clientHeight) >= scrollHeight && !animating) {
        if(pos < 4){
          openPos(pos+1)
        }
        setAnimating(true)
      }
      else if(scrollTop == 0 && !animating) {
        if(pos > 0){
          openPos(pos-1)
        }
        setAnimating(true)
